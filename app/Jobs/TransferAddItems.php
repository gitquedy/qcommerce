...
class TransferAddItems implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($set_of_item) {
            foreach ($set_of_item as $set) {
                ...
                $warehouse_set_quantity = $sku->computeSetWarehouseQuantity($this->item->to_warehouse_id);
                $warehouse_item = $sku->warehouse_items()->updateOrCreate(
                    ['warehouse_id' => $this->item->to_warehouse_id,
                    'sku_id' => $sku->id],
